(function() {
    ...
    function ProfileController(AccountService, $localStorage, $mdDialog, Upload, $timeout, $scope) {
        ...
        vm.request = request;
        if (vm.currentUser.roles[0] == "STUDENT" && vm.currentUser.degreeName !== "N/A") {
            vm.bool = true;
        }
        if (vm.currentUser.upgrade == 0){
            vm.disable = true;
        }

        function request(acID) {
            $mdDialog.show(
                $mdDialog.confirm()
                .title('Are You Sure?')
                .textContent('')
                ...
                AccountService.request(acID);
                vm.disable = true;
                $localStorage.currentUser.upgrade = 0;
            });
        }
