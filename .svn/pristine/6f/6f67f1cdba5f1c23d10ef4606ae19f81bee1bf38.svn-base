    ...
    .factory('AdminService', function($http) {
        return {
            ...
            createActivity: function(subject, numOfPe, startDate, endDate, content, grpmate, school){
                $http.get('http://unicomhk.net/path2/php/admin.php?action=createActivity&subject=' + subject + '&numOfPe=' + numOfPe + '&startDate=' + startDate + '&endDate=' + endDate + '&content=' + content + '&grpmate=' + grpmate + '&school=' + school);
            },
            updateActivity: function(subject, numOfPe, startDate, endDate, content, grpmate, id, school){
                $http.get('http://unicomhk.net/path2/php/admin.php?action=updateActivity&subject=' + subject + '&numOfPe=' + numOfPe + '&startDate=' + startDate + '&endDate=' + endDate + '&content=' + content + '&grpmate=' + grpmate + '&id=' + id + '&school=' + school);
            },
            deleteActivity: function(id) {
                $http.get('http://unicomhk.net/path2/php/admin.php?action=deleteActivity&id=' + id);
            },
            getLockList: function() {
                var query = $http.get('http://unicomhk.net/path2/php/admin.php?action=getLockList');
                return query;
            },
            unlock: function(lock) {
                $http.get('http://unicomhk.net/path2/php/admin.php?action=unlock&accountID=' + lock.accountID);
            },
            getRequestList:function(){
                var query = $http.get('http://unicomhk.net/path2/php/admin.php?action=getRequestList');
                return query;
            },
            upgrade:function(request){
                $http.get('http://unicomhk.net/path2/php/admin.php?action=upgrade&accountID=' + request.accountID);
            }
        };
    });
