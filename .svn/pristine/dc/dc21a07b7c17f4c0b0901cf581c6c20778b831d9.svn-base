(function() {
    ...
    /* @ngInject */
    function DefaultToolbarController($scope, $injector, $localStorage, $rootScope, $mdMedia, $state, $element, $filter, $mdUtil, $mdSidenav, $mdToast, $timeout, $document, triBreadcrumbsService, triSettings, triLayout) {
        var vm = this;
        ...
        var j = 0;
        var i = 0;
        vm.currentUser.notificationCount = 0;
        for (i = 0; i < vm.currentUser.notificationGroup.length; i++) {
            if (vm.currentUser.notificationGroup[i].notifications != null) {
                for (j = 0; j < vm.currentUser.notificationGroup[i].notifications.length; j++) {
                    if (vm.currentUser.notificationGroup[i].notifications[j].read == false) {
